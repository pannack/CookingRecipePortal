    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
      ...
      if(isset($_POST['submit'])) {
        ...
        if (move_uploaded_file($tempname, $folder)) {
            ...
            $insert = "INSERT INTO `recipe`(`sub_cat_id`,`prep_time`, `image_path`, `user_id`, `description`, `r_name`) VALUES ($category, '$recipe_dur', '$folder', $current, '$desc', '$title')";
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="container">
                ...
                <?php
                  ...
                  {
                    echo "<option disabled>".$row3['cat_name'];
                    $q2 = "SELECT * from sub_cat where cat_id = ".$row3['cat_id'];
                    $res2 = mysqli_query($link,$q2);
                    while($row2 = mysqli_fetch_assoc($res2))
                    {
                    echo '<option value="'.$row2['sub_cat_id'].'">** '.$row2['sub_cat_name'];
                    }
                ...
                <!-- Select Category:<br>
                    <select class="form-control" value="<?php if(isset($_POST["category"])) echo $_POST["category"]; ?>" name="category" required>
                  ...
                  $querydropdown = "SELECT cat_name from category";
                ...
                Select Sub Category:<br>
                  ...
                  <select class="form-control" value="<?php if(isset($_POST["sub_category"])) echo $_POST["sub_category"]; ?>" name="sub_category" required>
                    <option value="">Select an option</option>
                  <?php
                    $username =  $_SESSION['username'];
                    $querydropdown = "SELECT sub_cat_name from sub_cat";
